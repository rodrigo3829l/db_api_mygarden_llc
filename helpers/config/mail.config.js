...
export const getTemplate = (titleOne, titleTwo, prOne, prTwo, name, code, action, textAction) => {
    ...
    if (action === "confirm"){
        accion = `<a href="https://db-api-mygarden-llc.onrender.com/api/user/confirm/${code}" target="_blank">${textAction}</a>`
        // accion = `<a href="http://localhost:5000/api/user/confirm/${code}" target="_blank">${textAction}</a>`
    }
    else if (action === 'reactivated'){
        accion = `<a href="https://db-api-mygarden-llc.onrender.com/api/user/reactivate/${code}" target="_blank">${textAction}</a>`
